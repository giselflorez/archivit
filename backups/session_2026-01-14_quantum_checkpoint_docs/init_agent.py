...
# Import quantum checkpoint system
try:
    from quantum_checkpoint import QuantumCheckpoint
...
def check_quantum_recovery():
    """Check for crash recovery from quantum checkpoint system"""
    if not QUANTUM_AVAILABLE:
        return None

    try:
        checkpoint = QuantumCheckpoint(str(PROJECT_ROOT))
        recovery = checkpoint.get_recovery_context()
        return recovery
    except Exception as e:
        print(f"[Quantum] Error checking recovery: {e}")
        return None
...
def main():
    ...
    # Check for quantum crash recovery
    if QUANTUM_AVAILABLE:
        recovery = check_quantum_recovery()
        if recovery and recovery.get('crashed'):
            print("╔══════════════════════════════════════════════════════════════╗")
            print("║           🔄 CRASH RECOVERY DETECTED                         ║")
            print("╠══════════════════════════════════════════════════════════════╣")
            print(f"║ Last Focus: {str(recovery.get('lastFocus', 'Unknown'))[:48]:<48}║")
            print(f"║ Progress:   {str(recovery.get('progress', 0)) + '%':<48}║")
            print("║ Thought:                                                     ║")
            thought = str(recovery.get('thoughtFragment', 'None'))[:58]
            print(f"║   {thought:<58}║")
            print("║ Next Action Was:                                             ║")
            next_action = str(recovery.get('nextAction', 'Unknown'))[:58]
            print(f"║   {next_action:<58}║")
            print("╚══════════════════════════════════════════════════════════════╝")
            print()
